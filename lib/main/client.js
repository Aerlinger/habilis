...
function request(client, invocation, options) {
  ...
  const inputPromise = write(childProcess, _.assign({ id }, invocation))
  const successEvent = options.successEvent
  ...
  console.log("SUBMITTING REQUEST: ", invocation)
  ...
  return inputPromise
    ...
    .finally(function() {
      ...
      delete requestMap[id]; // clean up reference, no matter what the result
    });
}
