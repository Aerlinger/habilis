...
import { JupyterProcess, create, exec } from './JupyterProcess'

// const log = asInternal(__filename)
let log = console.log
let clientMap = {}

export function getPythonScriptResults(targetFile, options = {}) {
  log("Getting: ", targetFile)

  const processOptions = getPythonCommandOptions(options)
  const cmd            = options.cmd || 'python'

  log("Running: ", cmd, [targetFile], processOptions)

  return exec(cmd, [targetFile], processOptions)
}
...
function createPythonScriptProcess(targetFile, options) {
  ...
  return create(cmd, [targetFile], processOptions)
}
...
export class JupyterClient extends EventEmitter {
  ...
  constructor() {
    ...
    clientMap[this.childProcess.childProcess.pid] = this
