...
export function dispatchEvents(client) {
  let child = client.childProcess.childProcess

  // TODO: Use promises instead of events
  const objectEmitter = createObjectEmitter(client.childProcess.childProcess.stdout)
...
function handle(client, response) {
  if (isStartComplete(response)) {
    ...
    log('info', "RESPONSE RESULT/SOURCE:", response)
